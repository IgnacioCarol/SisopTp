...
echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Bienvenido al programa de instalación/reparación del sistema.-instalarTP.sh-$USER" >> ./instalarTP.log
...
then #Caso en el que no existe. Realizamos la instalación.
    ...
    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Comenzando configuración para la instalación del sistema.-instalarTP.sh-$USER" >> ./instalarTP.log
	...
	while [ "$instalar" = "No" ] ; do
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Ingrese un nombre para el directorio de ejecutables (en caso de dejar en blanco, se utilizará el nombre por default \"$ejecutables_default\"):-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-$ejecutables_temp-instalarTP.sh-$USER" >> ./instalarTP.log
	    ...
	    while grep -Fxq "$ejecutables_temp" $reservados || [[ $ejecutables_temp == *"."* ]] || [[ $ejecutables_temp == *"/"* ]] ; do
	        if grep -Fxq "$ejecutables_temp" $reservados; then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-El nombre \"$ejecutables_temp\" no está permitido debido a que el directorio ya existe.-instalarTP.sh-$USER" >> ./instalarTP.log
	        else
	            echo "No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios.-instalarTP.sh-$USER" >> ./instalarTP.log
	        fi
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Ingrese un nombre para el directorio de tablas del sistema (en caso de dejar en blanco, se utilizará el nombre por default \"$tablas_default\"):-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-$tablas_temp-instalarTP.sh-$USER" >> ./instalarTP.log
	    ...
	    while grep -Fxq "$tablas_temp" $reservados || grep -Fxq "$tablas_temp" $ingresados || [[ $tablas_temp == *"."* ]] || [[ $tablas_temp == *"/"* ]] ; do
	        ...
	        then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-El nombre \"$tablas_temp\" no está permitido debido a que el directorio ya existe.-instalarTP.sh-$USER" >> ./instalarTP.log
	        else
	            if grep -Fxq "$tablas_temp" $ingresados
	            then
	                echo "EL nombre \"$tablas_temp\" ya fue seleccionado para otro directorio."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-EL nombre \"$tablas_temp\" ya fue seleccionado para otro directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
	            else
	                echo "No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios.-instalarTP.sh-$USER" >> ./instalarTP.log
	            fi
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Ingrese un nombre para el directorio de novedades (en caso de dejar en blanco, se utilizará el nombre por default \"$novedades_default\"):-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-$novedades_temp-instalarTP.sh-$USER" >> ./instalarTP.log
	    ...
	    while grep -Fxq "$novedades_temp" $reservados || grep -Fxq "$novedades_temp" $ingresados || [[ $novedades_temp == *"."* ]] || [[ $novedades_temp == *"/"* ]] ; do
	        ...
	        then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-El nombre \"$novedades_temp\" no está permitido debido a que el directorio ya existe.-instalarTP.sh-$USER" >> ./instalarTP.log
	        else
	            if grep -Fxq "$novedades_temp" $ingresados
	            then
	                echo "EL nombre \"$novedades_temp\" ya fue seleccionado para otro directorio."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-EL nombre \"$novedades_temp\" ya fue seleccionado para otro directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
	            else
	                echo "No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios.-instalarTP.sh-$USER" >> ./instalarTP.log
	            fi
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Ingrese un nombre para el directorio de archivos rechazados (en caso de dejar en blanco, se utilizará el nombre por default \"$rechazados_default\"):-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-$rechazados_temp-instalarTP.sh-$USER" >> ./instalarTP.log
	    ...
	    while grep -Fxq "$rechazados_temp" $reservados || grep -Fxq "$rechazados_temp" $ingresados || [[ $rechazados_temp == *"."* ]] || [[ $rechazados_temp == *"/"* ]] ; do
	        ...
	        then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-El nombre \"$rechazados_temp\" no está permitido debido a que el directorio ya existe.-instalarTP.sh-$USER" >> ./instalarTP.log
	        else
	            if grep -Fxq "$rechazados_temp" $ingresados
	            then
	                echo "EL nombre \"$rechazados_temp\" ya fue seleccionado para otro directorio."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-EL nombre \"$rechazados_temp\" ya fue seleccionado para otro directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
	            else
	                echo "No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios.-instalarTP.sh-$USER" >> ./instalarTP.log
	            fi
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Ingrese un nombre para el directorio de lotes procesados (en caso de dejar en blanco, se utilizará el nombre por default \"$lotes_default\"):-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-$lotes_temp-instalarTP.sh-$USER" >> ./instalarTP.log
	    ...
	    while grep -Fxq "$lotes_temp" $reservados || grep -Fxq "$lotes_temp" $ingresados || [[ $lotes_temp == *"."* ]] || [[ $lotes_temp == *"/"* ]] ; do
	        ...
	        then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-El nombre \"$lotes_temp\" no está permitido debido a que el directorio ya existe.-instalarTP.sh-$USER" >> ./instalarTP.log
	        else
	            if grep -Fxq "$lotes_temp" $ingresados
	            then
	                echo "EL nombre \"$lotes_temp\" ya fue seleccionado para otro directorio."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-EL nombre \"$lotes_temp\" ya fue seleccionado para otro directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
	            else
	                echo "No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios.-instalarTP.sh-$USER" >> ./instalarTP.log
	            fi
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Ingrese un nombre para el directorio de resultados (en caso de dejar en blanco, se utilizará el nombre por default \"$resultados_default\"):-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-$resultados_temp-instalarTP.sh-$USER" >> ./instalarTP.log
	    ...
	    while grep -Fxq "$resultados_temp" $reservados || grep -Fxq "$resultados_temp" $ingresados || [[ $resultados_temp == *"."* ]] || [[ $resultados_temp == *"/"* ]] ; do
	        ...
	        then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-El nombre \"$resultados_temp\" no está permitido debido a que el directorio ya existe.-instalarTP.sh-$USER" >> ./instalarTP.log
	        else
	            if grep -Fxq "$resultados_temp" $ingresados
	            then
	                echo "EL nombre \"$resultados_temp\" ya fue seleccionado para otro directorio."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-EL nombre \"$resultados_temp\" ya fue seleccionado para otro directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
	            else
	                echo "No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios."
	                echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No está permitido el uso de los caracteres "/" o "." para el nombre de los directorios.-instalarTP.sh-$USER" >> ./instalarTP.log
	            fi
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Tipo de proceso:                          INSTALACIÓN-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio padre:                         $(dirname `pwd`)-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Ubicación script de instalación:          `pwd`/instalarTP.sh-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Log de la instalación:                    `pwd`/instalarTP.log-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Archivo de configuración:                 `pwd`/instalarTP.conf-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Log de inicialización:                    `pwd`/iniciarambiente.log-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Log del proceso principal:                `pwd`/pprincipal.log-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de ejecutables:                $(dirname `pwd`)/$ejecutables_default-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de tablas maestras:            $(dirname `pwd`)/$tablas_default-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de novedades:                  $(dirname `pwd`)/$novedades_default-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de novedades aceptadas:        $(dirname `pwd`)/$novedades_default/ok-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de rechazados:                 $(dirname `pwd`)/$rechazados_default-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de lotes procesados:           $(dirname `pwd`)/$lotes_default-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de transacciones:              $(dirname `pwd`)/$resultados_default-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de comisiones:                 $(dirname `pwd`)/$resultados_default/comisiones-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Estado de la instalación:                 LISTA-instalarTP.sh-$USER" >> ./instalarTP.log
	    
	    read -p "¿Confirma la instalación? (Y/N) `echo $'\n> '`" respuesta
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-¿Confirma la instalación? (Y/N)-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-$respuesta-instalarTP.sh-$USER" >> ./instalarTP.log
	    while [[ "$respuesta" != "Y" && "$respuesta" != "y" && "$respuesta" != "N" && "$respuesta" != "n" ]] ; do
	        echo "Respuesta invalida."
	        echo "`date '+%d/%m/%Y %H:%M:%S'`-ERR-Respuesta invalida.-instalarTP.sh-$USER" >> ./instalarTP.log
	        read -p "¿Confirma la instalación? (Y/N) `echo $'\n> '`" respuesta
	        echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-¿Confirma la instalación? (Y/N)-instalarTP.sh-$USER" >> ./instalarTP.log
	        echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-$respuesta-instalarTP.sh-$USER" >> ./instalarTP.log
	    done
	...
	echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Creando directorio de ejecutables.-instalarTP.sh-$USER" >> ./instalarTP.log
	...
	echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Creando directorio de tablas del sistema.-instalarTP.sh-$USER" >> ./instalarTP.log
	...
	echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Creando directorio de novedades.-instalarTP.sh-$USER" >> ./instalarTP.log
	...
	echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Creando directorio de archivos rechazados.-instalarTP.sh-$USER" >> ./instalarTP.log
	...
	echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Creando directorio de lotes procesados.-instalarTP.sh-$USER" >> ./instalarTP.log
	...
	echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Creando directorio de resultados.-instalarTP.sh-$USER" >> ./instalarTP.log
	...
	echo "INSTALACION-`date '+%d/%m/%Y %H:%M:%S'`-$USER" >> ./instalarTP.conf
	echo "Estado de la instalación:                     COMPLETADA"
	echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Estado de la instalación:                     COMPLETADA-instalarTP.sh-$USER" >> ./instalarTP.log
	...
	then #Caso en el que no hay que hacer la reparación.
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-El sistema se encuentra correctamente instalado y no necesita reparación alguna.-instalarTP.sh-$USER" >> ./instalarTP.log
	    cat ./instalarTP.conf
	    ...
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Tipo de proceso:                          REPARACIÓN-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio padre:                         $(dirname `pwd`)-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Ubicación script de instalación:          `pwd`/instalarTP.sh-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Log de la instalación:                    `pwd`/instalarTP.log-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Archivo de configuración:                 `pwd`/instalarTP.conf-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Log de inicialización:                    `pwd`/iniciarambiente.log-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Log del proceso principal:                `pwd`/pprincipal.log-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de ejecutables:                $DIRBIN-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de tablas maestras:            $DIRMAE-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de novedades:                  $DIRIN-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de novedades aceptadas:        $DIRIN_SUB-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de rechazados:                 $DIRRECH-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de lotes procesados:           $DIRPROC-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de transacciones:              $DIROUT-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Directorio de comisiones:                 $DIROUT_SUB-instalarTP.sh-$USER" >> ./instalarTP.log
	    echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Estado de la reparación:                  LISTA-instalarTP.sh-$USER" >> ./instalarTP.log
	    ...
	    if [[ "$respuesta" = "Y" || "$respuesta" = "y" ]]; then
	        ...
	        echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Comenzando reparación del sistema.-instalarTP.sh-$USER" >> ./instalarTP.log
	    
	        if [ ! -d "$DIRBIN" ]; then
	            echo "No se encontró el directorio de ejecutables $DIRBIN. Reparando directorio."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el directorio de ejecutables $DIRBIN. Reparando directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
                mkdir $DIRBIN
	        ...
	        if [ ! -d "$DIRMAE" ]; then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el directorio de tablas del sistema $DIRMAE. Reparando directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
                mkdir $DIRMAE
	        ...
	        if [ ! -d "$DIRIN" ]; then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el directorio de novedades $DIRIN. Reparando directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
                mkdir $DIRIN
                mkdir $DIRIN_SUB
	        fi
	        ...
	        if [ ! -d "$DIRIN_SUB" ]; then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el directorio de novedades aceptadas $DIRIN_SUB. Reparando directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
                mkdir $DIRIN_SUB
	        fi
	    
	        if [ ! -d "$DIRRECH" ]; then
	            echo "No se encontró el directorio de archivos rechazados $DIRRECH. Reparando directorio."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el directorio de archivos rechazados $DIRRECH. Reparando directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
                mkdir $DIRRECH
	        fi
	    
	        if [ ! -d "$DIRPROC" ]; then
	            echo "No se encontró el directorio de lotes procesados $DIRPROC. Reparando directorio."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el directorio de lotes procesados $DIRPROC. Reparando directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
                mkdir $DIRPROC
	        fi
	    
	        if [ ! -d "$DIROUT" ]; then
	            echo "No se encontró el directorio de resultados $DIROUT. Reparando directorio."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el directorio de resultados $DIROUT. Reparando directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
                mkdir $DIROUT
                mkdir $DIROUT_SUB
	        fi
	        ...
	        if [ ! -d "$DIROUT_SUB" ]; then
	            ...
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el directorio de comisiones $DIROUT_SUB. Reparando directorio.-instalarTP.sh-$USER" >> ./instalarTP.log
                mkdir $DIROUT_SUB
	        fi
	    
	        if [ ! -f "$DIRMAE"/comercios.txt ];then
	            echo "No se encontró el maestro de comercios. Reparando archivo."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el maestro de comercios. Reparando archivo.-instalarTP.sh-$USER" >> ./instalarTP.log
	            cp ../original/tablasmaestras/comercios.txt $DIRMAE/comercios.txt
	        fi
	    
	        if [ ! -f "$DIRMAE"/tarjetashomologadas.txt ]; then
	            echo "No se encontró el maestro de tarjetas. Reparando archivo."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el maestro de tarjetas. Reparando archivo.-instalarTP.sh-$USER" >> ./instalarTP.log
	            cp ../original/tablasmaestras/tarjetashomologadas.txt $DIRMAE/tarjetashomologadas.txt
	        fi
			
			if [ ! -f "$DIRBIN"/pprincipal.sh ]; then
	            echo "No se encontró el ejecutable del programa principal. Reparando archivo."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el ejecutable del programa principal. Reparando archivo.-instalarTP.sh-$USER" >> ./instalarTP.log
	            cp ../original/scripts/pprincipal.sh $DIRBIN/pprincipal.sh
	        fi
			
			if [ ! -f "$DIRBIN"/salida1.bash ]; then
	            echo "No se encontró el ejecutable de archivos de liquidación. Reparando archivo."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el ejecutable de archivos de liquidación. Reparando archivo.-instalarTP.sh-$USER" >> ./instalarTP.log
	            cp ../original/scripts/salida1.bash $DIRBIN/salida1.bash
	        fi
			
			if [ ! -f "$DIRBIN"/salida2.bash ]; then
	            echo "No se encontró el ejecutable de cálculo del costo del servicio. Reparando archivo."
	            echo "`date '+%d/%m/%Y %H:%M:%S'`-WAR-No se encontró el ejecutable de cálculo del costo del servicio. Reparando archivo.-instalarTP.sh-$USER" >> ./instalarTP.log
	            cp ../original/scripts/salida2.bash $DIRBIN/salida2.bash
	        fi
	    
	        echo "REPARACION-`date '+%d/%m/%Y %H:%M:%S'`-$USER" >> ./instalarTP.conf
	        echo "Estado de la reparación:                     COMPLETADA"
	        echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Estado de la reparación:                     COMPLETADA-instalarTP.sh-$USER" >> ./instalarTP.log
	        cat ./instalarTP.conf
	    else
	        echo "Cancelando proceso de reparación."
	        echo "`date '+%d/%m/%Y %H:%M:%S'`-INF-Cancelando proceso de reparación.-instalarTP.sh-$USER" >> ./instalarTP.log
	    fi	      
	fi
fi
